...
def get_recommendations_store(title, cosine_sim=cosine_sim):
    ...
    recommendations = store2.iloc[store_indices][['store_id','food_name','price','calo']]
    return recommendations



food_menu_3 = food_menu
food_menu_4 = food_menu

features = ['food_name','types_food', 'description']

food_menu_3 = food_menu_3[features]

def create_soup(x):
    return str(x['food_name']) + " " + str(x['types_food']) + " + str(x['description'])"
food_menu_3['soup'] = food_menu_3.apply(create_soup, axis=1)

count = CountVectorizer(stop_words = vietnamese_stop_words)
count_matrix = count.fit_transform(food_menu_3['soup'])

cosine_sim2 = cosine_similarity(count_matrix, count_matrix)

def name(food_menu_3):
    if pd.notna(food_menu_3['food_name']):
        return food_menu_3['food_name']
    else:
        return None

food_menu_3['name'] = food_menu_3.apply(name, axis=1)

food_menu_3 = food_menu_3.reset_index()
indices = pd.Series(food_menu_3.index, index = food_menu_3['name'])
...
def get_recommendations2(title, cosine_sim=cosine_sim2):
    ...
    recommendations = food_menu_4.iloc[food_indices][['store_id','food_name','price','calo']]
